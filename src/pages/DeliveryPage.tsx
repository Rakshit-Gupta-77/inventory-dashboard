...
import { ChevronRight, Plus, Search, FileOutput, Loader2, Trash2 } from "lucide-react";
    ...
    <div className="space-y-6">
      <header className="flex items-end justify-between">
        <div>
          ...
          <h1 className="text-3xl font-medium tracking-tighter">Deliveries</h1>
        </div>
        ...
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
          ...
          <DialogContent className="sm:max-w-[600px] bg-card border-border max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle className="text-xl font-medium tracking-tight">Create New Delivery</DialogTitle>
            </DialogHeader>
            <form onSubmit={handleSubmit} className="space-y-4 mt-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  <label className="label-upper text-[10px]">Reference</label>
                  <input required value={formData.reference} onChange={(e) => setFormData({...formData, reference: e.target.value})} className="w-full bg-secondary/50 border border-border rounded-md px-3 py-2 text-sm outline-none focus:ring-1 focus:ring-primary" placeholder="DLV-XXXX" />
                </div>
                <div className="space-y-2">
                  <label className="label-upper text-[10px]">Customer</label>
                  <input required value={formData.customer} onChange={(e) => setFormData({...formData, customer: e.target.value})} className="w-full bg-secondary/50 border border-border rounded-md px-3 py-2 text-sm outline-none focus:ring-1 focus:ring-primary" placeholder="Customer Name" />
                </div>
              </div>
                ...
                {formData.items.map((item, index) => (
                  <div key={index} className="grid grid-cols-12 gap-3 items-end bg-secondary/20 p-3 rounded-md border border-border/50">
                    ...
                    <div className="col-span-5 space-y-1.5">
                      <label className="text-[10px] text-muted-foreground uppercase">Source Location</label>
                      <select required value={item.source_location_id} onChange={(e) => updateItem(index, "source_location_id", e.target.value)} className="w-full bg-secondary/50 border border-border rounded-md px-2 py-1.5 text-xs outline-none focus:ring-1 focus:ring-primary">
              ...
              <DialogFooter className="mt-6">
                <button type="submit" disabled={createMutation.isPending} className="w-full bg-primary hover:bg-primary/90 text-primary-foreground py-2 rounded-md text-sm font-medium transition-all glow-primary btn-press flex items-center justify-center gap-2">
                  {createMutation.isPending ? <Loader2 size={16} className="animate-spin" /> : <FileOutput size={16} />}
                  Create Delivery
                </button>
              </DialogFooter>
            </form>
      ...
      <div className="card-surface overflow-hidden">
        <table className="w-full text-left">
          <thead><tr className="border-b border-border bg-secondary/30">
            {["Reference", "Customer", "Date", "Status", ""].map((h) => (
            ...
            {isLoading ? (
              ...
              filtered.map((d, i) => (
                <motion.tr key={d.id} initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }} className="hover:bg-secondary/30 transition-colors">
                  ...
                  <td className="px-5 py-3 text-sm">{d.customer}</td>
                  <td className="px-5 py-3 text-xs text-muted-foreground">{new Date(d.date).toLocaleDateString()}</td>
                  <td className="px-5 py-3">
                    <span className={`text-[10px] px-2 py-0.5 rounded border font-bold uppercase tracking-wider ${
                      d.status === "Done" ? "border-success/30 text-success bg-success/5" :
                      d.status === "Ready" ? "border-primary/30 text-primary bg-primary/5" :
              ...
              <tr><td colSpan={5} className="px-5 py-8 text-center text-sm text-muted-foreground">No deliveries found</td></tr>
            )}
          </tbody>
        </table>
      </div>
