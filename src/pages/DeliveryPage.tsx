...
import { ChevronRight, Plus, Search, FileOutput, Loader2, Trash2, User, Calendar, Truck, MoreVertical, CheckCircle2, Clock, AlertCircle, Package, MapPin } from "lucide-react";
...
export default function DeliveryPage() {
  ...
  const [formData, setFormData] = useState({
    reference: "",
    customer: "",
    items: [{ product_id: "", quantity: 1, source_location_id: "" }]
  });
  ...
  const validateMutation = useMutation({
    mutationFn: api.deliveries.validate,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["deliveries"] });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (formData.items.some(item => !item.product_id || !item.source_location_id)) {
  ...
  const addItem = () => {
    setFormData({
      ...formData,
      items: [...formData.items, { product_id: "", quantity: 1, source_location_id: "" }]
    });
  };
  ...
  const getStatusStyle = (status: string) => {
    switch (status) {
      ...
      case "Draft": return { color: "bg-muted/50 text-muted-foreground border-border", icon: FileOutput };
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <div className="flex items-center gap-2 text-muted-foreground mb-2">
            ...
            <span className="label-upper text-foreground">Outbound Shipments</span>
          </div>
          <h1 className="text-4xl font-semibold tracking-tight text-foreground/90">Deliveries</h1>
          <p className="text-muted-foreground text-sm mt-1">Manage customer shipments and outgoing stock.</p>
        </div>
        ...
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
          ...
          <DialogContent className="sm:max-w-[700px] p-0 overflow-hidden border-none shadow-2xl bg-card">
            <div className="p-6 bg-gradient-to-br from-primary/10 via-transparent to-transparent border-b border-border/50">
              <DialogHeader>
                <DialogTitle className="text-2xl font-bold tracking-tight">Create Delivery Order</DialogTitle>
                <p className="text-muted-foreground text-sm">Register new outgoing shipment for a customer.</p>
              </DialogHeader>
            </div>
            <form onSubmit={handleSubmit} className="p-6 space-y-6 overflow-y-auto max-h-[70vh]">
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <div className="relative group">
                    <FileOutput className="absolute left-3 top-3 text-muted-foreground/50 group-focus-within:text-primary transition-colors" size={18} />
                    <input required value={formData.reference} onChange={(e) => setFormData({...formData, reference: e.target.value})} className="w-full bg-secondary/30 border border-border/50 rounded-xl pl-10 pr-4 py-3 text-sm outline-none focus:border-primary/50 transition-all" placeholder="e.g. DLV-2024-001" />
                  </div>
                </div>
                <div className="space-y-2">
                  <label className="label-upper">Customer Name</label>
                  <div className="relative group">
                    <User className="absolute left-3 top-3 text-muted-foreground/50 group-focus-within:text-primary transition-colors" size={18} />
                    <input required value={formData.customer} onChange={(e) => setFormData({...formData, customer: e.target.value})} className="w-full bg-secondary/30 border border-border/50 rounded-xl pl-10 pr-4 py-3 text-sm outline-none focus:border-primary/50 transition-all" placeholder="e.g. Acme Corporation" />
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between border-b border-border/50 pb-2">
                  <h3 className="text-sm font-bold uppercase tracking-wider text-muted-foreground/80 flex items-center gap-2">
                    <Package size={16} /> Delivery Items
                  </h3>
                  ...
                  {formData.items.map((item, index) => (
                    ...
                    >
                      ...
                      <div className="col-span-4 space-y-2">
                        ...
                        <select required value={item.source_location_id} onChange={(e) => updateItem(index, "source_location_id", e.target.value)} className="w-full bg-card border border-border/50 rounded-lg px-3 py-2 text-sm outline-none focus:border-primary/50 transition-all cursor-pointer">
              ...
              <DialogFooter className="pt-4 border-t border-border/50">
                <button type="submit" disabled={createMutation.isPending} className="w-full bg-primary hover:bg-primary/90 text-primary-foreground py-4 rounded-xl text-sm font-bold transition-all shadow-lg shadow-primary/20 glow-primary flex items-center justify-center gap-2 active:scale-[0.98]">
                  {createMutation.isPending ? <Loader2 size={20} className="animate-spin" /> : <FileOutput size={20} />}
                  <span>Confirm & Create Delivery</span>
                </button>
              </DialogFooter>
            </form>
          </DialogContent>
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 bg-secondary/20 p-4 rounded-2xl border border-border/50">
        <div className="flex items-center gap-3 bg-card border border-border/50 rounded-xl px-4 py-2.5 w-full md:w-96 focus-within:border-primary/50 transition-all">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search deliveries by reference or customer..." className="bg-transparent outline-none text-sm w-full placeholder:text-muted-foreground/40" />
        </div>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-border/50 bg-muted/30">
                ...
                <th className="px-6 py-4 label-upper">Customer / Destination</th>
                    ...
                    >
                      <td className="px-6 py-5">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="font-mono-data text-sm font-bold text-primary tracking-tight">{d.reference}</span>
                        </div>
                      </td>
                      <td className="px-6 py-5">
                        <div className="flex items-center gap-2">
                          <div className="w-7 h-7 rounded-full bg-secondary flex items-center justify-center text-[10px] font-bold">
                            {d.customer.substring(0, 2).toUpperCase()}
                          </div>
                          <span className="text-sm font-medium text-foreground/90">{d.customer}</span>
                        </div>
                      </td>
                      <td className="px-6 py-5">
                        ...
                        <div className="flex items-center justify-end gap-2">
                          {d.status !== "Done" && d.status !== "Canceled" && (
                            <button 
                              ...
                              className="text-xs font-bold bg-primary hover:bg-primary/90 text-primary-foreground px-4 py-2 rounded-lg transition-all shadow-lg shadow-primary/20 active:scale-95 flex items-center gap-2"
                            >
                ...
                <tr>
                  <td colSpan={5} className="px-6 py-20 text-center">
                    <div className="flex flex-col items-center gap-3 text-muted-foreground/50">
                      ...
                      <p className="text-sm font-medium">No deliveries found matching your filters</p>
                    </div>
                  </td>
                </tr>
              )}
